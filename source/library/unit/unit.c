...
void Global_init(Global *g, const int argc, const char **argv) {
    ...
    static const arg_Option options[] = {
        ...
        {'v', "verbose", "Print more testing details.", NULL},
        {0}};
    const char *out[4] = {0};
...
void mode_test(Global *g) {
    ...
    printf(COLORIZE(COLOR_BLUE, "TOTAL")
               " Passed " COLORIZE(COLOR_GREEN, "%zu") ","
               " failed " COLORIZE(COLOR_RED, "%zu") ","
...
void unit_named_suite(Global *g, const char *name, FnSuite suite) {
    ...
    if (g->config.is_verbose) {
        printf(COLORIZE(COLOR_BLUE, "RESULT") " %s " COLORIZE(COLOR_BLUE, "::")
                   " Passed " COLORIZE(COLOR_GREEN, "%zu") ","
                   " failed " COLORIZE(COLOR_RED, "%zu") ","
                   " skipped " COLORIZE(COLOR_YELLOW, "%zu") ".\n\n",
